...
function handleAssessmentWsUpgrade(server) {
	...
	wss.on('connection', async (clientWs, request) => {
			...
			try {
				if (typeof data === 'string' || data instanceof Buffer) {
					...
					if (parsed.type === 'audio_chunk') {
						...
						sessionState.userRequestedEnd = true
		...
		clientWs.on('close', async () => {
			...
			// If tokens were not yet logged and session was active, save fallback metrics & token logs
			if (!sessionState.tokensLogged && sessionState.isVoiceSession && sessionState.sessionId) {
				sessionState.tokensLogged = true
				const durationSec = Math.max(1, Math.round((new Date() - sessionState.startedAt) / 1000))
				const userWords = sessionState.turns
				...
				prisma.learner_profiles.upsert({
					where: { user_id: sessionState.userId },
					update: {
						total_sessions: { increment: 1 },
						total_minutes: { increment: durationMinutes },
						current_track: sessionState.trackKey,
						current_chapter: sessionState.chapterKey,
						updated_at: new Date(),
					},
					create: {
				...
				const inputAudioSec = Math.round(sessionState.userAudioChunksCount * 0.128)
				const outputAudioSec = Math.round(sessionState.aiAudioChunksCount * 0.128)
				const promptTokens = Math.max(100, inputAudioSec * 25 + (sessionState.turns.length * 15))
				const completionTokens = Math.max(50, outputAudioSec * 32)
				logTokenUsage({
					userId: sessionState.userId,
					featureArea: 'voice_session',
					subFeature: 'live_audio',
					provider: 'google',
					modelName: GEMINI_MODEL,
					promptTokens,
					completionTokens,
					totalTokens: promptTokens + completionTokens,
					requestDurationMs: durationSec * 1000,
					status: 'success',
					metadata: { sessionId: sessionState.sessionId, durationSeconds: durationSec, wordsSpoken: userWords, exitReason: 'client_closed' }
				})
			}
		})
	})
	...
	for (const call of calls) {
		...
		if (name === 'log_error') {
			...
			if (sessionState.currentUserText && sessionState.currentUserText.trim().length > 1) {
				...
				if (sessionState.isVoiceSession) {
					prisma.voice_session_turns.create({
						data: {
							session_id: sessionState.sessionId,
							sequence: sessionState.turnSequence,
			...
			try {
				...
				// Update learner profile: total_sessions & total_minutes
				const durationMinutes = Math.max(1, Math.round(durationSec / 60))
				prisma.learner_profiles.upsert({
					where: { user_id: sessionState.userId },
					update: {
				...
				}).catch(err => console.error('[Voice WS] Error updating learner profile:', err))

				// Log Token Usage for Live Voice Audio
				if (!sessionState.tokensLogged) {
					sessionState.tokensLogged = true
					const inputAudioSec = Math.round(sessionState.userAudioChunksCount * 0.128)
					const outputAudioSec = Math.round(sessionState.aiAudioChunksCount * 0.128)
					const promptTokens = Math.max(200, inputAudioSec * 25 + (sessionState.turns.length * 20))
					const completionTokens = Math.max(100, outputAudioSec * 32)
